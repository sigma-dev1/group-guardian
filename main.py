...
async def delete_messages_after_time(client, chat_id):
...
async def welcome_and_mute(client, message):
    for new_member in message.new_chat_members:
        ...
        message_ids[message.chat.id].append(welcome_message.message_id)
    ...
    if ip_address:
        ...
        if country_code != "IT":
            ...
            if duplicate_users:
                ...
                if message.chat.id not in message_ids:
                    message_ids[message.chat.id] = []
                message_ids[message.chat.id].append(confirmation_message.message_id)
...
for group_id in GROUP_IDS:
    asyncio.run(delete_messages_after_time(bot, group_id))
